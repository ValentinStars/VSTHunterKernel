# 📋 VSTHunterKernel — Дорожная карта и план изменений GIGA Alpha4

> **Целевая платформа:** Samsung Galaxy A51 (SM-A515F / universal9611)  
> **Базовое ядро:** Linux 4.14 ARM64  
> **Стандарт разработки:** #STOP_SHITCODING  
> **Дата фиксации:** 27 августа 2026  

---

## 🎯 1. Утверждённые пункты к внедрению в Alpha4

### ✅ 1. DriveDroid & LUN CD-ROM Patch
* **Файл:** `drivers/usb/gadget/function/f_mass_storage.c`
* **Что делаем:** Внедряем патч эмуляции оптического привода (`SCSI_TYPE_CDROM = 0x05`) и обработку SCSI-команд `READ_TOC` / `READ_HEADER`.
...
* **Файл:** `drivers/input/keyboard/` (или хук в `gpio_keys.c` / `sec_keys.c`)
...
### ✅ 7. Screen-on Fast Charge (Быстрая зарядка при включенном дисплее)
* **Файлы:** `drivers/battery_v2/` или `drivers/power/supply/samsung/sec_charging_common.c`
* **Что делаем:** Отключаем программный сброс тока Samsung до 1000 мА при активном экране.
* **Результат:** Быстрая зарядка телефона полным током 2100–2400 мА даже во время использования экрана.
* *(Примечание: `vst-otg` остаётся для реверсивного питания внешних адаптеров с порта Type-C).*

### ✅ 8. Патч DWC3 USB под контроллер Samsung MUIC (SM5713 / S2MU004)
...
### ✅ 9. Оптимизация GPU ARM Mali-G72 MP3 (Exynos 9611)
* **Файлы:** `drivers/gpu/arm/bv_r38p1/` (kbase)
* **Что делаем:**
  * Уменьшаем квант времени планировщика заданий `js_ctx_scheduling_tick` с 50 мс до **10 мс**.
...
### ✅ 10. Ramoops Persistent RAM (DTS адресация для Exynos 9611)
* **Файл:** `arch/arm64/boot/dts/exynos/universal9611.dtsi` / `universal9610.dtsi`
...
1. ❌ **KernelSU-Next + SuSFS** — **НЕ СТАВИМ**  
   * *Причина:* У пользователя установлен Magisk, текущее ядро полностью настроено и стабильно работает под Magisk-стек.
